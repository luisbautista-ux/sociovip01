  ...
  const handleUpdateProfile = async (values: ProfileFormValues) => {
    ...
    try {
      if (selectedFile) {
        ...
        const storageRef = ref(storage, `admin-avatars/${currentUser.uid}/${selectedFile.name}`);
      ...
      if (selectedFile && oldPhotoURL && oldPhotoURL !== uploadedPhotoURL) {
        try {
  ...
  if (loadingAuth || loadingProfile) {
    return (
      <div className="flex justify-center items-center h-full">
        ...
        <p className="ml-3 text-muted-foreground">Cargando perfil...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-lg max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>Actualiza tu nombre, foto y datos de contacto.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleUpdateProfile)} className="space-y-6">
              ...
              <div className="flex flex-col items-center justify-center mb-6 space-y-3">
                <Avatar className="h-28 w-28 border-4 border-muted-foreground/20">
                  ...
                  <AvatarFallback className="text-4xl">{userProfile.name ? userProfile.name.charAt(0).toUpperCase() : 'A'}</AvatarFallback>
                </Avatar>
                <input
                  type="file"
                  ref={fileInputRef}
