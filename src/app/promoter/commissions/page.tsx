...
import type { PromoterCommissionEntry, Business, BusinessManagedEntity } from "@/lib/types";
...
export default function PromoterCommissionsPage() {
  ...
  const fetchCommissions = useCallback(async () => {
    ...
    try {
        ...
        // Fetch business details to populate the filter dropdown
        ...
        // Fetch all entities for the assigned businesses
        ...
        entitiesSnap.forEach(doc => {
            ...
            // Filter codes generated by the current promoter
            const promoterCodes = (entity.generatedCodes || []).filter(c => c.generatedByUid === userProfile.uid);
            
            promoterCodes.forEach(code => {
                // A commission is generated if the code was redeemed (QR generated) or used (at the door)
                if ((code.status === 'redeemed' || code.status === 'used') && code.commissionGenerated) {
                     allCommissions.push({
                        ...
                        promoterCodesRedeemed: 1, // Each row represents one code use/commission
                        commissionRateApplied: `S/ ${code.commissionGenerated.toFixed(2)}`,
                        commissionEarned: code.commissionGenerated,
        ...
        setCommissionData(allCommissions.sort((a,b) => new Date(b.period).getTime() - new Date(a.period).getTime()));
  ...
  const { totalEarned, totalPaid, totalPending } = useMemo(() => {
    const totalEarned = filteredCommissions.reduce((sum, comm) => sum + comm.commissionEarned, 0);
    const totalPaid = filteredCommissions.filter(c => c.paymentStatus === 'Pagado').reduce((sum, comm) => sum + comm.commissionEarned, 0);
        ...
        <CardContent>
                    ...
                    {filteredCommissions.map((comm) => (
                      <TableRow key={comm.id}>
                        ...
                        <TableCell className="text-center">
                            <Badge variant={comm.paymentStatus === 'Pagado' ? 'default' : 'secondary'} className={comm.paymentStatus === 'Pagado' ? 'bg-green-600' : ''}>
                                {comm.paymentStatus}
                            </Badge>
                        </TableCell>
                      </TableRow>
         ...
         <CardFooter className="flex flex-col sm:flex-row items-center justify-between pt-4 border-t gap-4">
             <div className="text-lg font-bold">
                Total Pendiente: <span className="text-destructive">S/ {totalPending.toFixed(2)}</span>
             </div>
